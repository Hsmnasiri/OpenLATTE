...
# Each "family" (stem) goes entirely to train or test, and both _debug/_stripped are kept together.

set -euo pipefail

# ---------- CONFIG ----------
# ریشهٔ Juliet (برای پیش‌فرض‌ها)
export JULIET_ROOT="/mnt/z/juliet/c"
# مسیر باینری‌های ساخته‌شده (اسکریپت قبلی آن‌ها را در build/<CWE>/sXX گذاشت)
SRC_ROOT="${SRC_ROOT:-$JULIET_ROOT/build}"

# مقصد دیتاست
DEST_ROOT="${DEST_ROOT:-$JULIET_ROOT/dataset}"

# لیست CWEهایی که می‌خوای تقسیم شوند
CWES=(
...
# تعداد خانواده‌هایی که برای هر CWE به train می‌روند (باقی به test)
N_TRAIN="${N_TRAIN:-90}"

# بذر تصادفی (اختیاری)
SEED="${SEED:-42}"

# اگر True باشد و DEST_ROOT وجود داشته باشد، پاکش می‌کند
FORCE="${FORCE:-false}"

# ---------- UTILS ----------
have_shuf() { command -v shuf >/dev/null 2>&1; }
rand_shuffle() {
  # ورودی: خطوط لیست؛ خروجی: همان لیست به‌صورت تصادفی
  if have_shuf; then
    SHUF_SEED_OPT=()
    # BSD shuf ممکنه SEED نداشته باشه؛ اگر نشد بدون SEED می‌ریم
...
for CWE in "${CWES[@]}"; do
  ...
  # همهٔ فایل‌های اجرایی را پیدا کن
  ...
  # خانواده‌ها (stem) را استخراج کن: حذف پسوند _debug/_stripped
  declare -A families=()
  for f in "${files[@]}"; do
    base="$(basename "$f")"
    dir="$(dirname "$f")"
  ...
  # انتخاب تصادفی N_TRAIN (اگر N_TRAIN>total، همه به train می‌روند)
  n_pick="$N_TRAIN"
  ...
  for stem_path in "${train_fams[@]}"; do
    # دو فایل ممکن: _debug و _stripped
    dbg="$stem_path"_debug
    str="$stem_path"_stripped
