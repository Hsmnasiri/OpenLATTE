...
# -*- coding: utf-8 -*-
"""
Batch-classify sinks via local LLM API (classify.py).

Usage:
  python batch_classify_sinks.py \
...
def main():
    ...
    out_file  = os.path.join(args.output_dir,
                             f"sink_classification_{bin_name}.json")

    results = []
    for item in externals:
        resp = query_sink(item["name"])
    ...
    print(f"[DONE] {len(results)} sink(s) saved to {out_file}")
