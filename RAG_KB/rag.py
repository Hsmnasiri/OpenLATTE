...
# -*- coding: utf-8 -*-
"""
inspect_flows_with_llm.py - Implements LATTE Phase 3 with RAG enhancement.
"""
import json
import requests
import argparse
...
# --- Prompt Templates from LATTE Paper ---
...
# --- NEW: RAG-Enhanced Final Prompt Template ---
...
def main():
    ...
    # --- NEW: Argument to enable RAG mode and provide the KB file ---
    parser.add_argument('--rag', help="Path to a knowledge base file to enable RAG mode.")
    args = parser.parse_args()

    # --- NEW: Load the RAG knowledge base if provided ---
    rag_context = None
    if args.rag:
        try:
    ...
    for i, flow in enumerate(dangerous_flows):
        ...
        # We will use Gemini for this advanced task
        query_llm = query_gemini

        # 1. Start Prompt (same as before)
        ...
        # 2. Middle Prompts (same as before)
        ...
        # 3. End Prompt (Now with RAG logic)
        if rag_context:
            # If RAG is enabled, use the augmented prompt
            end_prompt = RAG_END_PROMPT_TEMPLATE.format(rag_context=rag_context)
        else:
            # Otherwise, use the original LATTE prompt
